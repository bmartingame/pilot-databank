...
function normalizeMapEntryName(value) {
  return String(value || "")
    ...
    .replace(/[’‘]/g, "'")
    .toLowerCase()
    .replace(/\s+(system|sector)\s*$/i, "")
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
}

function entryMatchesMapKind(entry, kind) {
...
export default function App() {
  ...
  async function handleMapNodeOpen({ name, kind }) {
    ...
    if (!requestedName) {
      throw new Error("The selected map node has no name.");
    }

    const strippedName = requestedName
      .replace(/\s+(system|sector)\s*$/i, "")
      .trim();
    ...
    const results = [...resultMap.values()];
    const targetName = normalizeMapEntryName(requestedName);

    const exactNameMatches = results.filter(
      (entry) => normalizeMapEntryName(entry?.name) === targetName
    );

    const match =
      exactNameMatches.find((entry) => entryMatchesMapKind(entry, kind)) ||
      exactNameMatches[0] ||
      results.find(
        (entry) =>
          entryMatchesMapKind(entry, kind) &&
          normalizeMapEntryName(entry?.name).includes(targetName)
      ) ||
      results.find(
        (entry) =>
          entryMatchesMapKind(entry, kind) &&
          targetName.includes(normalizeMapEntryName(entry?.name))
      );

    if (!match) {
      throw new Error(
        `No ${String(kind || "map").toLowerCase()} databank entry matches "${requestedName}".`
      );
    }
